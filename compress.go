...
// 压缩后的图片路径与原路径相同
// 返回错误信息
func CompressImage(filePath string, thumbFilepath string, quality int) error {
	// 打开文件
	...
	if err != nil {
		fmt.Printf("打开目标图片异常:%s", err)
		return err
	}
	// 复制图片
	if _, err = io.Copy(dst, file); err != nil {
		fmt.Printf("复制图片异常:%s", err)
		return err
	}
	...
	case "jpeg", "jpg":
		if contentType != "image/jpeg" {
			fmt.Printf("不支持的图片格式:%s", contentType)
			return err
		}
		// 重新打开文件以写入压缩后的图片
		...
		defer file.Close()
		dstImage := imaging.Resize(img, 400, 0, imaging.Lanczos)
		// 使用JPEG格式压缩图片
